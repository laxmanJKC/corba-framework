 ...
 * <p/>
 * After initialization of a {@link NioSslClient} object,
 * {@link NioSslClient#connect()} should be called, in order to establish
 * connection with the server.
 * <p/>
 * When the connection between the client and the object is established,
 * {@link NioSslClient} provides a public write and read method, in order to
...
public class NioSSLClient extends NioSSLPeer {
	 ...
	 *                      with up to TLSv1 protocol. Java 1.7 or higher also
	 *                      supports TLSv1.1 and TLSv1.2 protocols.
	...
	public NioSSLClient(String protocol, String remoteAddress, int port) throws Exception {
		...
		KeyStore clientStore = KeyStore.getInstance("PKCS12");
		 clientStore.load(new FileInputStream("src/main/resources/keystore.p12"), "changeit".toCharArray());
		//clientStore.load(new FileInputStream("src/main/resources/baeldung.cer"), "changeit".toCharArray());
	 ...
	 * engine are inner class' variables. {@link NioSslClient#write(String)} should
	 * be called instead.
	 *
	 * @param message - message to be sent to the server.
	 ...
	 * <p/>
	 * Just like {@link NioSslClient#read(SocketChannel, SSLEngine)} it uses inner
	...
	protected void read(SocketChannel socketChannel, SSLEngine engine) throws Exception {
		...
		LOGGER.debug("About to read from the server...");
		...
		while (!exitReadLoop) {
			...
			if (bytesRead > 0) {
				...
				while (peerNetData.hasRemaining()) {
					...
					case OK:
						peerAppData.flip();
						LOGGER.debug("Server response: " + new String(peerAppData.array()));
						exitReadLoop = true;
						break;
					case BUFFER_OVERFLOW:
	...
	public void shutdown() throws IOException {
		LOGGER.debug("About to close connection with the server...");
		closeConnection(socketChannel, engine);
		executor.shutdown();
